...
/**
 * Get provider level by ID
 * @param int $id Provider level ID
 * @return array|bool Provider level data or false if not found
 */
function get_provider_level($id) {
    global $db;
    $db->query('SELECT * FROM provider_levels WHERE id = :id');
    $db->bind(':id', $id);
    return $db->single();
}